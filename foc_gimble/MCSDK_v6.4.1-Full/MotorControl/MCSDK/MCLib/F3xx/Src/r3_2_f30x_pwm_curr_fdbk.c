 ...
 *
 * @brief 3-Shunt, 2 ADCs, PWM & Current Feedback implementation for F30X, F4XX, F7XX, G4XX, H5XX, H7XX and L4XX MCUs
 *
...
/**
  ...
  *         in three shunt topology shared ADC.
  * 
...
/**
  ...
  * Specific to F30X, G4XX and H7XX.
  * 
  * @param  TIMx: Timer instance peripheral.
...
/**
  ...
  *
  * @param  pHdl: Handler of the current instance of the PWM component.
  * @retval uint16_t Returns the value of R3_2_WriteTIMRegisters.
  */
uint16_t R3_2_SetADCSampPointPolarization( PWMC_Handle_t * pHdl )
{
#if defined (__ICCARM__)
...
#pragma cstat_restore = "MISRAC2012-Rule-11.3"
  ...
  else
  {
    ...
    else
    {
      ...
      {
        /* ADC trigger edge must be changed from positive to negative */
        pHandle->ADCTriggerEdge = (uint16_t) LL_ADC_INJ_TRIG_EXT_FALLING;
        SamplingPoint = pHdl->lowDuty + pHandle->pParams_str->Tbefore;
      }
      else
      {
...
__weak void * R3_2_TIMx_UP_IRQHandler( PWMC_R3_2_Handle_t * pHandle )
{
  ...
  /* We can not change OPAMP source if ADC acquisition is ongoing (Dual motor with internal opamp use case)*/  
    ...
    {}
      ...
      MODIFY_REG(operationAmp->CSR, OPAMP_CSR_VPSEL, OpampConfig);
...
/**
  ...
  *
  * Specific for Motor Profiling using Shunt-based current sensing.
  *
  * @param  pHdl: Handler of the current instance of the PWM component.
  */
